#' Generate the estimation results about simulation analysis by Maxent, GM, rGM and Fisher by the Broyden-Fletcher-Goldfarb-Shanno (BFGS) algorithm 
#'
#' \code{est.sim} returns the results of simulation analysis by Maxent, GM, rGM and Fisher

#' @param rho a parameter in correlation matrix 

#' @param p sample size

#' @param m presence sample size

#' @param alpha0 parameter to be estimated in log-linear model




#' @return the values of test AUC and computational costs by Maxent, GM, rGM and Fisher



#' @export

est.sim <-function (rho = 0.5, n.grid = 100, m = 500, p = 50, 
                     alpha0 = seq(0, 0.1, length.out = p), type = "poisson") {
  N <- n.grid^2
  library(MASS)
  ...
  maxent.loss <- function(beta) {
    -mean(Xb.p %*% beta) + log(sum(exp(Xb %*% beta)))
  }
  maxent.loss.gr <- function(beta) {
    -apply(Xb.p, 2, mean) + 
      apply(as.numeric(exp(Xb %*% beta)) * Xb, 2, sum) / sum(exp(Xb %*% beta))
  }

  rgm.loss <- function(beta, gamma) {
    sum(exp(gamma * t(t(Xb.p) - fbar) %*% beta - 
              1/2 * gamma * (gamma + 1) * as.numeric(t(beta) %*% S %*% beta)))
  }
  rgm.loss.gr <- function(beta, gamma) {
    apply(as.numeric(exp(gamma * t(t(Xb.p) - fbar) %*% beta - 
                           1/2 * gamma * (gamma + 1) * as.numeric(t(beta) %*% S %*% beta))) *
            (gamma * t(t(Xb.p) - fbar - (gamma + 1) * as.numeric(S %*% beta))), 2, sum)
  }
